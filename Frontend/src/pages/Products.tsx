...
export function Products() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // Asegurarse de que los precios sean números antes de enviar
      const productToSend = {
        ...product,
        precio_usd: Number(product.precio_usd),
        precio_eur: Number(product.precio_eur),
        precio_cop: Number(product.precio_cop)
      };
      
      await api.createProduct(productToSend);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name.includes('precio')) {
      // Si el campo está vacío, establecer como string vacío para permitir borrar el 0
  ...
  const handleFocus = (e: React.FocusEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    if (name.includes('precio') && value === '0') {
      setProduct(prev => ({
        ...prev,
        [name]: ''
      }));
